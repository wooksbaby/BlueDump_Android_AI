...
def classify_images_with_options(target_directory, images_directory, output_base_directory):
    ...
    for align in align_options:
		...
		# 매칭되지 않은 이미지를 저장할 디렉토리 생성
        #unmatched_directory = os.path.join(option_directory, "unmatched")
        #ensure_directory_exists(unmatched_directory)

        for image in images:
            #matched = False  # 각 이미지가 매칭되었는지 여부
            ...
            # 매칭되지 않은 이미지는 unmatched 디렉토리에 복사
            #if not matched:
            #    shutil.copy(image, unmatched_directory)
            #    print(f"{os.path.basename(image)}는 타겟 이미지와 일치하지 않아 {unmatched_directory}에 저장되었습니다.")
