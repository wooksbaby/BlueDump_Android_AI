...
# 이미지를 배치로 처리하는 함수
def process_images_in_batches(target_images, images, model_name, distance_metric, detector_backend, align, batch_size, option_directory):
    """주어진 배치 크기로 이미지를 처리"""
    image_count = 0
    for i in range(0, len(images), batch_size):
        batch_images = images[i:i + batch_size]  # 배치 크기만큼 이미지를 가져옴
        ...
        for distance_metric in distance_metrics:
            for detector_backend in detector_backends:
                for align in align_options:
                    ...
                    image_count = process_images_in_batches(target_images, images, model_name, distance_metric, detector_backend, align, batch_size, option_directory)  # 배치 처리 함수 호출
...
output_directory = os.path.join(current_directory, "classified_images")

# 사용 예시
classify_images_with_options(target_directory, images_directory, output_directory, batch_size=128)  # 배치 크기를 1로 설정
